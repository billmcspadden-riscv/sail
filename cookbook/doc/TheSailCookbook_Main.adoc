...
Sail has a built-in `bit` type with members `bitzero` and `bitone`. Note: 

- `bit` is **not** a numeric type, i.e. not `range(0, 1)`. 
- 'bit`  is **not** the type `bool`

### Bitvectors and bits


For decrementing bits we define:

`bitvector('n, dec)` is hardcoded as an abbreviation for `vector('n, dec, bit)` and likewise for  `bitvector('n, inc)`. Now we can define `bits(.)` as follows.
...
This class is parameterised by `T` which is a placeholder for a
type. We can instatiate `T` by other types. We call parameters like
`T` a *type variable*. In Java we also have generics, e.g.
